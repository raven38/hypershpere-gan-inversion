...
from training.projectors import w_plus_projector, z_plus_projector
...
class BaseCoach:
    ...
    def load_inversions(self, w_path_dir, image_name):
        ...
        if not os.path.isfile(w_potential_path):
            print('not find ', w_potential_path)
            return None
        #breakpoint()
    ...
    def calc_inversions(self, image, pose,image_name):
        ...
        if hyperparameters.first_inv_type == 'w+':
            ...
            w = z_plus_projector.project(self.G, id_image, pose, device=torch.device(global_config.device), w_avg_samples=600,
    ...
    def forward(self, w,pose,eval):
        ...
        if hyperparameters.first_inv_type == 'z+':
            ...
            conditioning_params = conditioning_params.expand([14,conditioning_params.shape[1]])
            self.G.backbone.mapping.num_ws = None
            w = self.G.mapping(w, conditioning_params, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff)
            w = w.view(1, self.G.backbone.num_ws, self.G.w_dim)        
