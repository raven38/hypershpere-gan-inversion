...
def save_single_image_z(base_dir, image_latent, G, file_name):
    image_to_save = get_image_from_z(image_latent, G)
    image_to_save = Image.fromarray(image_to_save, mode='RGB')
    image_to_save.save(f'{base_dir}/{file_name}.jpg')
...
def get_image_from_z(w, G):
    G.backbone.mapping.num_ws = None
    if w.ndim == 3:
        w = w.squeeze(0)
    w = G.mapping(w, None)
    w = w.view(1, G.backbone.num_ws, G.w_dim)
    with torch.no_grad():
        img = G.synthesis(w, noise_mode='const')
        img = (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8).detach().cpu().numpy()
    return img[0]
