...
def main():
    ...
    for batch_idx in tqdm(range(image_num)):
        ...
        # detailcode_starting = detailcode_starting.view(1, -1)
        ...
        # zs = torch.randn(1, generator.z_space_dim).cuda()
        zs = generator.mapping.norm(zs)
        zs.requires_grad_(True)
        generator.repeat_w = False
        generator.truncation.repeat_w = False
        # generator.mapping.normalize_input = False
        # generator.mapping.norm = nn.Identity()
        ...
        optimizing_variable.append(zs)
        basecode_layer, basecode = None, None
        if basecode_layer is not None:
        ...
        for iter in pbar:
            loss = 0.
            w = generator.mapping(zs, None)['w']
            w = w.view(1, generator.num_layers, generator.z_space_dim)
            # w = w.view(1, 1, generator.z_space_dim)
            wp = generator.truncation(w, trunc_psi=args.trunc_psi, trunc_layers=args.trunc_layers)
            # wp = generator.truncation(detailcode, trunc_psi=args.trunc_psi, trunc_layers=args.trunc_layers)
            # wp = torch.cat([detailcode, w], 1)
            ...
            if False and args.weight_pnorm_term:
            ...
            optimizer.zero_grad()
            loss.backward()
            pbar.set_postfix(loss=f'{loss.item():.5f}', lpips_loss=f'{lpips_loss.item():.5f}', mse_loss=f'{mse_loss.item():.5f}')
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            detailcode_save = zs.clone().detach().cpu().numpy()
