...
def main():
    ...
    for filename in glob(f'{inversion_dir}/invert_results/*.png'):
        ...
        detailcodes = np.concatenate([detailcodes, detailcode[:, :generator.num_layers, :]], axis=0)
    ...
    for sam_id in tqdm(range(len(image_list)), leave=False):
        ...
        interpolates = linear_interpolate(detailcode, boundary, args.start_distance, args.end_distance, args.step)
        ...
        for interpolation_id, interpolate in enumerate(interpolates):
            ...
            temp_code = temp_code.unsqueeze(0)
            temp_code = generator.truncation(temp_code, trunc_psi=args.trunc_psi, trunc_layers=args.trunc_layers)
